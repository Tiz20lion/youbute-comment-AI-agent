...
class OAuth2Setup(BaseModel):
    action: str = Field(..., description="OAuth2 action: generate_url, complete_auth")
...
async def manage_oauth2_setup(config: OAuth2Setup, request: Request):
    ...
    try:
        if config.action == "generate_url":
            ...
            raise HTTPException(status_code=400, detail="Invalid action. Use 'generate_url' or 'complete_auth'")
    ...
    try:
        ...
        if error:
            ...
            <html>
                ...
                <body style="font-family: Arial, sans-serif; padding: 20px; background: #1f2937; color: white;">
                    <h1>âŒ OAuth2 Authentication Failed</h1>
                    <p>Error: {error}</p>
                    ...
                    <script>
                        ...
                        // Enhanced window closing for errors
                        let attempts = 0;
                        function attemptClose() {{{{
                            ...
                            try {{{{
                                window.close();
                                setTimeout(() => {{{{
                                    if (attempts < 5) attemptClose();
                                }}}}, 500);
                            }}}} catch (e) {{{{
                                if (attempts < 5) setTimeout(attemptClose, 500);
                            }}}}
                        }}}}
        ...
        try:
            ...
            if success:
                ...
                <html lang="en">
                    ...
                    <body>
                        ...
                        <div class="container">
                            ...
                            <p class="subtitle">Your YouTube account has been securely linked to the AI Comment Bot</p>
                            
                            <div class="features">
                                <div class="feature-item">
                                    <span class="icon">✅</span>
                                    <span>OAuth2 Authentication Complete</span>
                                </div>
                                <div class="feature-item">
                                    <span class="icon">🤖</span>
                                    <span>AI Comment Posting Enabled</span>
                                </div>
                                <div class="feature-item">
                                    <span class="icon">🔒</span>
                            ...
                            <div class="countdown">
                                <div>This window will close automatically in</div>
                        ...
                        <script>
                            ...
                            function updateCountdown() {{{{
                                ...
                                if (countdownValue > 0) {{{{
                                    ...
                                    // Countdown finished, change message and start closing immediately
                                    countdownElement.textContent = 'Closing...';
                                    startClosingAttempts();
                                }}}}
                            }}}}
                            
                            // Enhanced postMessage handling for COOP compliance
                            function notifyParent() {
                                try {{{{
                                    ...
                                    if (window.opener && !window.opener.closed) {
                                        ...
                                        // Also try common parent origins
                                        window.opener.postMessage({
                                            type: 'oauth2_complete',
                                            success: true
                                        }, 'http://localhost:7844');
                                        
                                        console.log('OAuth2 success message sent to parent');
                                    }
                                }}}} catch (e) {{{{
                                    console.log('Could not communicate with parent window (COOP policy):', e);
                                    
                                    // Fallback: Use localStorage for communication
                                    try {{{{
                                        localStorage.setItem('oauth2_result', JSON.stringify({
                                            type: 'oauth2_complete',
                                            success: true,
                                            timestamp: Date.now()
                                        }));
                                        console.log('OAuth2 result stored in localStorage');
                                    } catch (storageError) {
                                        console.log('localStorage fallback failed:', storageError);
                                    }
                                }
                            }
                            ...
                            document.addEventListener('DOMContentLoaded', function() {{{{
                                ...
                                function attemptClose() {{{{
                                    attempts++;
                                    console.log(`🔄 Attempt ${{attempts}}/${{maxAttempts}} to close window`);
                                    ...
                                    try {{{{
                                        ...
                                        // If we're still here after a delay, try again
                                        setTimeout(() => {{{{
                                            if (attempts < maxAttempts && !window.closed) {{{{
                                                console.log(`🔄 Window still open, trying again...`);
                                                attemptClose();
                                            }}}} else if (attempts >= maxAttempts) {{{{
                                                console.log(`❌ Max attempts reached, showing close button`);
                                                showCloseButton();
                                            }}}}
                                        }}}}, 800);
                                        
                                    }}}} catch (e) {{{{
                                        console.log(`❌ Close attempt ${{attempts}} failed:`, e);
                                        if (attempts < maxAttempts) {{{{
                                            setTimeout(attemptClose, 800);
                                        }}}} else {{{{
                                            console.log(`❌ All attempts failed, showing close button`);
                                            showCloseButton();
                                        }}}}
                                    }}}}
                                }}}}
                                
                                // Start closing attempts when countdown finishes (removed duplicate setTimeout)
                                // setTimeout(startClosingAttempts, 3200); // Removed - now triggered by countdown
                                
                                // Show close button if auto-close doesn't work
                                function showCloseButton() {{{{
                                    console.log('Showing manual close button');
                                    ...
                                    closeButton.onclick = () => {{{{
                                        try {{{{
                                            ...
                                            alert('Please close this window manually by clicking the X button in your browser tab.');
                                        }}}}
                                    }}}};
                                    
                                    const countdown = document.querySelector('.countdown');
                                    countdown.innerHTML = '<div style="margin-bottom: 15px;">Please close this window manually:</div>';
                                    countdown.appendChild(closeButton);
                                }}}}
                                
                                // Also try immediate close for browsers that allow it
                                setTimeout(() => {{{{
                                    try {{{{
                                        window.close();
                                    }}}} catch (e) {{{{
                                        console.log('Immediate close failed, waiting for countdown');
                                    }}}}
                                }}}}, 100);
                            }}}});
                        </script>
                ...
                error_msg = "Failed to complete the authentication process"
                return HTMLResponse(f"""
                <html>
                    <head><title>OAuth2 Error</title></head>
                    <body style="font-family: Arial, sans-serif; padding: 20px; background: #1f2937; color: white;">
